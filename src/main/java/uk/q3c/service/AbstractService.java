...
public abstract class AbstractService implements Service {
    ...
    public synchronized ServiceStatus stop(Cause cause) {
        if (state == State.STOPPED || state == State.STOPPING || state == State.FAILED || state == State.RESETTING) {
    ...
    public synchronized ServiceStatus reset() {
        if (state == State.INITIAL || state == State.RESETTING) {
            return new ServiceStatus(this, this.state, this.cause);
        }
        ...
        try {
            doReset();
            setState(State.INITIAL, Cause.RESET);
            return new ServiceStatus(this, this.state, this.cause);
        } catch (Exception e) {
